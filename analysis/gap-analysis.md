...
1. **Incorporate into existing IR training (IR-2 base).** Most CSPs/agencies already run IR training annually. Add a breach module covering breach definition, recognition, reporting channels (state CSA, CJIS Systems Officer, FBI CJIS Division per IR-6 implementation-level delta), and time-sensitive obligations.
...
- **Ties to IR-6.** IR-6 implementation-level delta added reporting to CSO and FBI CJIS Division. IR-2.3 training reinforces the reporting chain; staff should know who to call, at what number, within what timeframe, with what information.
...
3. **Applicable privacy requirements identification** — catalog the privacy statutes, regulations, or policies that apply to a given breach type (for CJI: CJIS Security Policy Section 5.13, state breach notification statutes, Privacy Act exemptions, agency-specific requirements).
...
2. **Define the notice determination process.** Decision tree: what data was exposed, to whom, for how long, with what likelihood of misuse. Outcome: notify individuals? notify state CSA? notify FBI CJIS Division? notify state attorney general (if state breach law applies)? notify media?
...
- **CJIS Security Policy Section 5.13 (Incident Response and Reporting).** Provides specific CJIS breach reporting requirements including timeframes for initial notification and content requirements for breach reports. The IR Plan should reference section 5.13 explicitly and map its workflow to those requirements.
- **Interplay with IR-6 implementation-level delta.** IR-6 covers who is notified and the reporting channels (US-CERT plus CSO plus FBI CJIS Division). IR-8.1 covers the process the plan documents for breach response. The two controls are complementary; both should be explicitly mapped in the plan.
